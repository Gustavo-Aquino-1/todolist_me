  ...
  return (
    <div className={`w-[100%] min-h-[100vh] flex items-center flex-col gap-[50px] p-20 relative ${theme == 'light' ? 'bg-white' : 'bg-gray-900'}`}>
      ...
      <div className="text-xl gap-10 w-[100%]" id="tasks">
        {tasks?.map((e) => (
          <div
            className={`flex gap-10 border-2 w-[30%] max-md:w-[70%] max-sm:w-[80%] m-auto  justify-between overflow-x-hidden rounded-md px-4 py-3 ${(isSeeingAll || isRegistering || editing) && 'blur-sm'} ${theme == 'light' ? 'bg-gray-200  border-blue-900 hover:bg-gray-500 hover:text-white' : 'bg-gray-900 text-white border-blue-800 hover:bg-blue-800'}`}
            key={e?.id}
          >
            <p onClick={() => {
                setActualTask(e)
