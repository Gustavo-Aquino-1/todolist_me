...
function Form({ editing, id, setIsRegistering, setIsEditing }) {
  ...
  return (
    <div className="overlay">
        <button className="w-[100%] flex justify-end text-2xl mr-20 mt-5 text-red-500 hover:text-red-700" onClick={() => {
            setIsEditing(false)
            setIsRegistering(false)
        }}>x</button>
      ...
      <form className="task_form w-[100%] h-[100%]" onSubmit={editing ? edit : create}>
        <label className="w-[90%] m-auto flex items-center justify-center">
          ...
          <input className="w-[100%] px-4 py-3 rounded-md outline-none" ref={task} type="text" />
        </label>

        {editing && (
          <label className="w-[90%] m-auto flex items-center justify-center">
            <p className="w-[100%] text-xl">Status</p>
            <select className="w-[100%] rounded-md  px-4 py-3" ref={status}>
        ...
        <button className="bg-blue-950 text-white text-xl w-[50%] m-auto px-4 py-2 rounded-md self-end" type="submit">{editing? "Edit": "Create"}</button>
      </form>
    </div>
  );
}
